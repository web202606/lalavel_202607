...
@extends('base')
@section('head')
<title>{{ config('app.name', 'laravelスキル登録画面') }}</title>
@endsection
@section('main')
    <main>
                    ...
                    <div class="row gx-5 justify-content-center">
                        <div class="col-md-8">
                            <div class="text-center my-5">
                                <h1 class="display-5 fw-bolder text-white mb-3  text-nowrap">laravelスキル登録画面</h1>
                                <p class="lead text-white-50 mb-1 text-nowrap">laravelスキルを登録してください</p>
        ...
        <div class="card-body d-flex justify-content-center">
            <form action="{{ route('laraveltbls.store') }}" method="post">
                ...
                <select name='laraveltbl_route' class='form-control'>
                    @if(empty(old('laraveltbl_route'))) or (old('laraveltbl_route') == 3) 
                        <option value="3" selected>よく理解している</option>
                    @else
                        <option value="3">よく理解している</option>
                    @endif
                    @if(old('laraveltbl_route') == 2) 
                        <option value="2" selected>まずまず理解している</option>
                    @else
                        <option value="2">まずまず理解している</option>
                    @endif                        
                    @if(old('laraveltbl_route') == 1) 
                        <option value="1" selected>理解不足</option>
                    @else
                        <option value="1">理解不足</option>
                    @endif
                ...
                <select name='laraveltbl_controller' class='form-control'>
                    @if(empty(old('laraveltbl_controller'))) or (old('laraveltbl_controller') == 3) 
                        <option value="3" selected>よく理解している</option>
                    @else
                        <option value="3">よく理解している</option>
                    @endif
                    @if(old('laraveltbl_controller') == 2) 
                        <option value="2" selected>まずまず理解している</option>
                    @else
                        <option value="2">まずまず理解している</option>
                    @endif                        
                    @if(old('laraveltbl_controller') == 1) 
                        <option value="1" selected>理解不足</option>
                    @else
                        <option value="1">理解不足</option>
                    @endif
                ...
                <select name='laraveltbl_model' class='form-control'>
                    @if(empty(old('laraveltbl_model'))) or (old('laraveltbl_model') == 3) 
                        <option value="3" selected>よく理解している</option>
                    @else
                        <option value="3">よく理解している</option>
                    @endif
                    @if(old('laraveltbl_model') == 2) 
                        <option value="2" selected>まずまず理解している</option>
                    @else
                        <option value="2">まずまず理解している</option>
                    @endif                        
                    @if(old('laraveltbl_model') == 1) 
                        <option value="1" selected>理解不足</option>
                    @else
                        <option value="1">理解不足</option>
                    @endif
                ...
                <select name='laraveltbl_view' class='form-control'>
                    @if(empty(old('laraveltbl_view'))) or (old('laraveltbl_view') == 3) 
                        <option value="3" selected>よく理解している</option>
                    @else
                        <option value="3">よく理解している</option>
                    @endif
                    @if(old('laraveltbl_view') == 2) 
                        <option value="2" selected>まずまず理解している</option>
                    @else
                        <option value="2">まずまず理解している</option>
                    @endif                        
                    @if(old('laraveltbl_view') == 1) 
                        <option value="1" selected>理解不足</option>
                    @else
                        <option value="1">理解不足</option>
                    @endif
                ...
                <select name='laraveltbl_naming' class='form-control'>
                     @if(empty(old('laraveltbl_naming'))) or (old('laraveltbl_naming') == 3) 
                        <option value="3" selected>よく理解している</option>
                    @else
                        <option value="3">よく理解している</option>
                    @endif
                    @if(old('laraveltbl_naming') == 2) 
                        <option value="2" selected>まずまず理解している</option>
                    @else
                        <option value="2">まずまず理解している</option>
                    @endif                        
                    @if(old('laraveltbl_naming') == 1) 
                        <option value="1" selected>理解不足</option>
                    @else
                        <option value="1">理解不足</option>
                    @endif
                ...
                <select name='laraveltbl_eloquent' class='form-control'>
                    @if(empty(old('laraveltbl_eloquent'))) or (old('laraveltbl_eloquent') == 3) 
                        <option value="3" selected>よく理解している</option>
                    @else
                        <option value="3">よく理解している</option>
                    @endif
                    @if(old('laraveltbl_eloquent') == 2) 
                        <option value="2" selected>まずまず理解している</option>
                    @else
                        <option value="2">まずまず理解している</option>
                    @endif                        
                    @if(old('laraveltbl_eloquent') == 1) 
                        <option value="1" selected>理解不足</option>
                    @else
                        <option value="1">理解不足</option>
                    @endif
                ...
                <select name='laraveltbl_join' class='form-control'>
                    @if(empty(old('laraveltbl_join'))) or (old('laraveltbl_join') == 3) 
                        <option value="3" selected>よく理解している</option>
                    @else
                        <option value="3">よく理解している</option>
                    @endif
                    @if(old('laraveltbl_join') == 2) 
                        <option value="2" selected>まずまず理解している</option>
                    @else
                        <option value="2">まずまず理解している</option>
                    @endif                        
                    @if(old('laraveltbl_join') == 1) 
                        <option value="1" selected>理解不足</option>
                    @else
                        <option value="1">理解不足</option>
                    @endif
                ...
                <select name='laraveltbl_templete' class='form-control'>
                    @if(empty(old('laraveltbl_templete'))) or (old('laraveltbl_templete') == 3) 
                        <option value="3" selected>よく理解している</option>
                    @else
                        <option value="3">よく理解している</option>
                    @endif
                    @if(old('laraveltbl_templete') == 2) 
                        <option value="2" selected>まずまず理解している</option>
                    @else
                        <option value="2">まずまず理解している</option>
                    @endif                        
                    @if(old('laraveltbl_templete') == 1) 
                        <option value="1" selected>理解不足</option>
                    @else
                        <option value="1">理解不足</option>
                    @endif
                ...
                <select name='laraveltbl_web' class='form-control'>
                    @if(empty(old('laraveltbl_web'))) or (old('laraveltbl_web') == 3) 
                        <option value="3" selected>よく理解している</option>
                    @else
                        <option value="3">よく理解している</option>
                    @endif
                    @if(old('laraveltbl_web') == 2) 
                        <option value="2" selected>まずまず理解している</option>
                    @else
                        <option value="2">まずまず理解している</option>
                    @endif                        
                    @if(old('laraveltbl_web') == 1) 
                        <option value="1" selected>理解不足</option>
                    @else
                        <option value="1">理解不足</option>
                    @endif
